...
- نموذج إضافة المنتج في Flutter يظهر كنافذة سفلية ثابتة بارتفاع 95% عبر `showGeneralDialog`. لا تستخدم `showModalBottomSheet` أو `DraggableScrollableSheet` لهذا النموذج الطويل؛ كلاهما سبب تعارض/تجمّد سكرول أو نافذة فارغة في Flutter Web. يحافظ Dialog Route على شكل النافذة دون Gesture السحب الخاص بالـbottom sheet.
- محتوى نموذج المنتج الطويل يستخدم `SingleChildScrollView` مع `Column` بدل `ListView` لتجنب إعادة تدوير وإعادة تخطيط حقول الإدخال أثناء السكرول في Flutter Web debug. كما يصرّح `MaterialApp.scrollBehavior` باللمس والماوس والقلم والتراكباد.

...
- متابعة `409`: أثبت اختبار Auth بعيد حقيقي أن Supabase يقبل `A1234567` ثم حُذف حساب الاختبار، لذلك `409` تعارض بيانات لا سياسة كلمة مرور. وُحّد تحقق اسم المستخدم مع قيد القاعدة إلى 4–40 محرفًا بلا مسافات، وأضيف فحص مسبق ورسائل مستقلة للبريد والجوال واسم المستخدم المسجل، مع تحويل أخطاء Auth غير المتعارضة إلى `500` وتسجيل رمز الخطأ داخليًا. نجح TypeScript وESLint واختبار تعارض بعيد بحساب مؤقت مع تنظيفه.

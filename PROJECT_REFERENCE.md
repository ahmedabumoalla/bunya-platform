...
- نموذج إضافة المنتج في Flutter نافذة سفلية عادية بارتفاع 95% و`ListView` داخلي، مع `enableDrag: false` للنافذة حتى يستلم النموذج حركة السكرول. لا تستخدم `DraggableScrollableSheet` هنا لأنه أدى إلى نافذة فارغة في Flutter Web.

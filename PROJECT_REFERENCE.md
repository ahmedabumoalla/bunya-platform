...
- نموذج إضافة المنتج في Flutter يستخدم نافذة `showModalBottomSheet` الأصلية بارتفاع 95% و`ListView`. لا تستبدل مسار النافذة بـ`showGeneralDialog` لأنه ترك طبقة سوداء عند تعذر رسم المحتوى في Flutter Web. القياسات تُضاف واحدًا تلو الآخر بزر إضافة وتظهر كصفوف خفيفة قابلة للحذف، بينما خيارات الضغط/الكثافة ونحوها تُدخل كنص متعدد القيم ثم تتحول عند الحفظ إلى `product_measurements` و`product_variants`. تجنب `InputChip` والقوائم الديناميكية الثقيلة داخل النافذة.

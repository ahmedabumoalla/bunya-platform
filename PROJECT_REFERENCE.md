...
- نموذج إضافة المنتج في Flutter يستخدم نافذة `showModalBottomSheet` الأصلية بارتفاع 95% و`ListView`. لا تستبدل مسار النافذة بـ`showGeneralDialog` لأنه ترك طبقة سوداء عند تعذر رسم المحتوى في Flutter Web. تعدد المقاسات والخيارات يُدخل كنص متعدد القيم مفصول بفاصلة (مثل `الضغط: 16، الضغط: 20`) ثم يحوّل عند الحفظ إلى صفوف `product_measurements` و`product_variants`؛ تجنب شرائح الإدخال والقوائم الديناميكية الثقيلة داخل هذه النافذة.

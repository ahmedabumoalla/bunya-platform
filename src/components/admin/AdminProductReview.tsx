...
type Measurement = { label: string; is_default: boolean; sort_order: number };
type Variant = {
  name: string;
  sku: string;
...
const decisionCopy: Record<ReviewDecision, { title: string; description: string; submit: string }> = {
  approved: {
    title: "اعتماد المنتج ونشره",
    description: "سيظهر المنتج للعملاء فور حفظ القرار.",
    submit: "تأكيد الاعتماد والنشر",
  },
  needs_changes: {
    title: "إعادة المنتج للتعديل",
    description: "لن يُنشر المنتج، وستصل ملاحظاتك إلى المزوّد ليصحح البيانات.",
    submit: "إرسال طلب التعديلات",
  },
  rejected: {
    title: "رفض المنتج",
    description: "لن يُنشر المنتج، وسيُحفظ سبب الرفض في سجل المراجعة.",
    submit: "تأكيد رفض المنتج",
  },
};
...
export function AdminProductReview({ initialProductId }: { initialProductId?: string }) {
  ...
  const counts = useMemo(
    () => ({
      pending: products.filter((item) => item.review_status === "pending_review").length,
      approved: products.filter((item) => item.review_status === "approved").length,
      other: products.filter((item) => !["pending_review", "approved"].includes(item.review_status)).length,
    }),
    [products],
  );
  ...
  async function submitReview() {
    ...
    setSaving(true);
    setError("");
    const result = await createClient().rpc("review_product", {
  ...
  return (
    <main className={`${styles.page} database-page`}>
            ...
            return (
              <article className={styles.card} key={product.id}>
                <div className={styles.visual}>
                  {productImage?.signed_url ? (
                    <Image src={productImage.signed_url} alt={productImage.alt_text || product.name} fill sizes="(max-width: 700px) 100vw, 35vw" unoptimized />
                  ) : (
                ...
                <div className={styles.body}>
                  <header className={styles.heading}>
                    <div>
                      <small>
                        {product.providers?.company_name || "منشأة غير محددة"} · {product.product_categories?.name || product.custom_category || "بلا تصنيف"}
                      </small>
                      <h2>{product.name}</h2>
                    </div>
...
function ReviewButtons({ onChoose }: { onChoose: (decision: ReviewDecision) => void }) {
  return (
    <div className={styles.reviewButtons} aria-label="إجراءات مراجعة المنتج">
      <button className={styles.approveButton} type="button" onClick={() => onChoose("approved")}>
        اعتماد ونشر
      </button>
      <button className={styles.changesButton} type="button" onClick={() => onChoose("needs_changes")}>
        طلب تعديلات
      </button>
      <button className={styles.rejectButton} type="button" onClick={() => onChoose("rejected")}>
        رفض
      </button>
    </div>
  );
    ...
    >
      <section className={styles.modal} role="dialog" aria-modal="true" aria-labelledby="admin-product-title">
        ...
        <header className={styles.modalHeader}>
          <div>
            <small>
              {product.providers?.company_name || "منشأة غير محددة"} · {product.product_categories?.name || product.custom_category || "بلا تصنيف"} · {product.sku || "بلا SKU"}
            </small>
            <h2 id="admin-product-title">{product.name}</h2>
          </div>
          <span className={`${styles.badge} ${badgeTone(product.review_status)}`}>{label(product.review_status)}</span>
        </header>
        {product.review_status === "pending_review" ? (
          <section className={styles.decisionArea}>
            <div className={styles.decisionHeading}>
              <div>
                ...
                <span>اختر الإجراء بعد فحص الصور والبيانات أدناه.</span>
              </div>
              <ReviewButtons onChoose={onChooseDecision} />
            </div>
            {reviewDecision ? (
              <div className={`${styles.reviewForm} ${styles[reviewDecision]}`}>
                <div>
                  <h3>{decisionCopy[reviewDecision].title}</h3>
                  <p>{decisionCopy[reviewDecision].description}</p>
                </div>
                <label htmlFor="product-review-reason">ملاحظة القرار {reviewDecision === "approved" ? "" : "(ستظهر للمزوّد)"}</label>
                <textarea id="product-review-reason" rows={3} value={reason} onChange={(event) => onReason(event.target.value)} disabled={saving} placeholder={reviewDecision === "approved" ? "ملاحظة الاعتماد" : "اكتب المطلوب من المزوّد بوضوح"} />
                {error ? <div className={styles.inlineError}>{error}</div> : null}
                <div className={styles.confirmRow}>
                  <button className={styles.confirmDecision} type="button" onClick={onSubmit} disabled={saving}>
                    {saving ? "جارٍ حفظ القرار…" : decisionCopy[reviewDecision].submit}
                  </button>
                </div>
              </div>
            ) : null}
          ...
          <div className={styles.readOnlyDecision}>تم اتخاذ قرار على هذا المنتج. يمكنك مراجعة التفاصيل وسجل القرارات أدناه.</div>
        )}
            ...
            <div className={styles.heroImage}>
              {shown?.signed_url ? (
                ...
                <div className={styles.fallback}>
                  <div>
                    <b>{product.name.slice(0, 1)}</b>
                    <span>لم يرفع المزوّد صورة لهذا المنتج</span>
                  </div>
                </div>
              )}
            </div>
          ...
          <div className={styles.sections}>
            <section className={styles.panel}>
              ...
              <dl className={styles.detailFacts}>
                ...
                <Fact name="المخزون" value={product.stock_quantity === null ? "غير محدد" : `${number(product.stock_quantity, 3)} ${product.base_unit}`} />
                <Fact name="نوع العرض" value={label(product.offer_type)} />
                <Fact name="حالة النشر" value={product.is_published ? "منشور" : "غير منشور"} />
                {product.offer_type === "rental" ? <Fact name="مدة التأجير" value={`${number(product.rental_duration_value)} ${product.rental_duration_unit || ""}`} /> : null}
              </dl>
            </section>
            <section className={styles.panel}>
              <h3>الوصف الكامل</h3>
              <p className={styles.text}>{product.full_description || product.description || product.short_description || "لا يوجد وصف."}</p>
            </section>
            {product.product_measurements.length || product.product_variants.length ? (
              <section className={styles.panel}>
                <h3>المقاسات والفئات المتوفرة</h3>
                <dl className={styles.detailFacts}>
                  {product.product_measurements.map((item) => (
                    <Fact key={`${item.label}-${item.sort_order}`} name={item.is_default ? "القياس الافتراضي" : "قياس متوفر"} value={item.label} />
                  ))}
                  {product.product_variants.map((item) => (
                    <Fact key={item.sku} name="خيار منتج" value={item.name} />
                  ))}
                </dl>
              </section>
            ) : null}
              ...
              <section className={styles.panel}>
                ...
                <dl className={styles.detailFacts}>
                  {product.product_specifications.map((item) => (
                    <Fact key={`${item.value}-${item.sort_order}`} name="مواصفة" value={item.value} />
                  ))}
                </dl>
              </section>
            ) : null}
            {product.product_warranties ? (
              <section className={styles.panel}>
                <h3>الضمان</h3>
                <dl className={styles.detailFacts}>
                  <Fact name={product.product_warranties.label} value={product.product_warranties.duration} />
                  <Fact name="تفاصيل الضمان" value={product.product_warranties.details} />
                </dl>
              </section>
            ) : null}
            <section className={styles.panel}>
              <h3>التوفر والتوصيل</h3>
              <dl className={styles.detailFacts}>
                <Fact name="حالة التوفر" value={label(product.availability_status)} />
                <Fact name="ملخص التوفر" value={product.availability_summary} />
                <Fact name="مدة التجهيز" value={product.lead_time_label} />
                <Fact name="طريقة التوصيل" value={product.delivery_label} />
                <Fact name="نافذة التوصيل" value={product.delivery_window} />
                <Fact name="ملاحظات التوصيل" value={product.delivery_notes} />
              </dl>
            </section>
            {history.length ? (
              <section className={styles.panel}>
                <h3>سجل قرارات المراجعة</h3>
                <div className={styles.history}>
                  {history.map((item, index) => (
                    <div key={`${item.reviewed_at}-${index}`}>
                      <b>{label(item.outcome)}</b>
                      <span>{item.reason}</span>
                      <small>{date(item.reviewed_at)}</small>
                    </div>
                  ))}
                </div>
              </section>

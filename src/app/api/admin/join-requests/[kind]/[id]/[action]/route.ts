...
export async function POST(
  request: NextRequest,
  context: { params: Promise<{ kind: string; id: string; action: string }> },
) {
  ...
  if (
    !["provider", "contractor"].includes(kind) ||
    ![
      "approve",
      "reject",
      "needs-changes",
  ...
  if (requiresReason && reason.length < 5)
    return NextResponse.json(
      { message: "سبب القرار مطلوب (خمسة أحرف على الأقل)." },
      { status: 400 },
    );
  const table =
    kind === "provider" ? "provider_applications" : "contractor_applications";
  const application = await auth.admin
    .from(table)
    .select("*")
    .eq("id", id)
    .maybeSingle();
  if (application.error) {
    ...
    return NextResponse.json(
      { message: "تعذر قراءة طلب الانضمام من قاعدة البيانات." },
      { status: 500 },
    );
  }
  if (!application.data)
  ...
  const relations = await loadApplicationRelations(
    auth.admin,
    kind as "provider" | "contractor",
    id,
  );
  if (relations.error) {
    logDataError("application_relations", relations.error, kind, id);
    return NextResponse.json(
      { message: "تعذر قراءة تفاصيل طلب الانضمام من قاعدة البيانات." },
      { status: 500 },
    );
  }
  ...
  if (action === "reject" || action === "needs-changes") {
    if (application.data.status === "approved")
      return NextResponse.json(
        { message: "لا يمكن تغيير طلب معتمد بهذه العملية." },
        { status: 409 },
      );
    ...
    if (updated.error || !updated.data)
      return NextResponse.json(
        { message: "تعذر تسجيل القرار أو سبق تنفيذه." },
        { status: 409 },
      );
    ...
    if (action === "reject") {
      await sendJoinApplicantDecision({
        ...
        mobile: application.data.mobile,
        reason,
        idempotencyKey: `join-rejected-${kind}-${id}`,
      });
    }
    if (action === "needs-changes") {
      ...
      if (created.error)
        return NextResponse.json(
          { message: "تعذر إنشاء رابط التعديل." },
          { status: 500 },
        );
      ...
      await sendJoinApplicantDecision({
        ...
        mobile: application.data.mobile,
        reason,
        revisionUrl: `${site}/join/revise/${token}`,
        idempotencyKey: key,
      });
    }
  ...
  if (action === "resend-credentials") {
    const onboarding = await auth.admin
      .from("account_onboarding_deliveries")
      .select("auth_user_id")
      .eq("application_kind", kind)
      .eq("application_id", id)
      .maybeSingle();
    if (!onboarding.data?.auth_user_id)
      return NextResponse.json(
    ...
    const delivery = await sendOnboardingCredentials({
      ...
      mobile: application.data.mobile,
      password,
      idempotencyKey: `onboarding-resend-${id}-${crypto.randomUUID()}`,
    });
    await confirmApprovedJoinPhone(
      auth.admin,
      onboarding.data.auth_user_id,
  ...
  if (!["pending", "needs_changes"].includes(application.data.status))
    return NextResponse.json(
      { message: "الطلب غير قابل للموافقة أو سبق تنفيذه." },
      { status: 409 },
    );
  const existing = await auth.admin
    .from("account_onboarding_deliveries")
    .select("auth_user_id,provisioning_status")
    .eq("application_kind", kind)
    .eq("application_id", id)
    .maybeSingle();
  if (existing.data?.auth_user_id)
    return NextResponse.json(
      { message: "تم إنشاء الحساب لهذا الطلب مسبقًا." },
      { status: 409 },
    );
  ...
  if (kind === "provider") {
    ...
    if (!username.value) {
      return NextResponse.json(
        {
          message:
            "اسم المستخدم المطلوب غير صالح أو مستخدم مسبقًا. اطلب تعديله ثم أعد الموافقة.",
        },
        { status: 409 },
      );
    }
    if (username.value !== application.data.requested_username) {
      ...
      if (updatedUsername.error || !updatedUsername.data) {
        ...
        return NextResponse.json(
          { message: "تعذر تجهيز اسم المستخدم للحساب." },
          { status: 500 },
        );
      }
  ...
  if (created.error || !created.data.user)
    return NextResponse.json(
      { message: "تعذر إنشاء حساب الدخول؛ تحقق من عدم تعارض البريد." },
      { status: 409 },
    );
  const userId = created.data.user.id;
  const rpcName =
    kind === "provider"
      ? "finalize_provider_application_approval"
      : "finalize_contractor_application_approval";
  const finalized = await auth.admin.rpc(rpcName, {
    p_application_id: id,
    p_auth_user_id: userId,
    p_reviewer_id: auth.userId,
    p_reason: "تمت الموافقة على طلب الانضمام.",
  });
  if (finalized.error) {
  ...
  const delivery = await sendOnboardingCredentials({
    ...
    mobile: application.data.mobile,
    password,
    idempotencyKey: `onboarding-approved-${id}`,
  });
  await confirmApprovedJoinPhone(
    auth.admin,
    userId,
...
async function recordReview(
  ...
  await admin
    .from("audit_logs")
    .insert({
      actor_profile_id: reviewerId,
      entity_table: `${kind}_applications`,
      entity_id: id,
      action: `join_application_${outcome}`,
      new_data: { reason },
    });
}
async function markDelivery(
  ...
  kind: string,
  id: string,
  delivery: Awaited<ReturnType<typeof sendOnboardingCredentials>>,
) {
  await admin.rpc("mark_onboarding_credentials_delivery", {
    p_application_kind: kind,
    p_application_id: id,
    p_email_status: delivery.email.status,
    p_whatsapp_status: delivery.whatsapp.status,
    p_email_reference: delivery.email.providerMessageId,
    p_whatsapp_reference: delivery.whatsapp.providerMessageId,
    p_error: delivery.email.sanitizedError || delivery.whatsapp.sanitizedError,
  });
}
function deliveryStatus(
  delivery: Awaited<ReturnType<typeof sendOnboardingCredentials>>,
) {
  const submitted = [delivery.email.status, delivery.whatsapp.status].filter(
    (v) => v === "submitted",
  ).length;
  return submitted === 2
    ? "credentials_submitted"
    : submitted === 1
      ? "credentials_partially_submitted"
      : "credentials_failed";
}

async function confirmApprovedJoinPhone(
  admin: ReturnType<typeof import("@/lib/supabase/admin").createAdminClient>,
  userId: string,
  mobile: unknown,
  whatsappStatus: string,
) {
  if (whatsappStatus !== "submitted") return;
  const phone = normalizeSaudiPhone(mobile);
  if (!phone) return;
  ...
  if (updated.error)
    console.error("[join-requests] approved_phone_confirmation", {
      userId,
      code: updated.error.code ?? "unknown",
    });
}
...
async function loadApplicationRelations(
  ...
  if (kind === "provider") {
    const [categories, regions] = await Promise.all([
      ...
      admin
        .from("provider_delivery_regions")
        .select("region_name")
        .eq("application_id", applicationId),
    ]);
...
function logDataError(
  ...
  console.error(`[join-requests] ${stage}`, {
    kind,
    id,
...
function applicationDetails(
  ...
  if (kind === "provider") {
    const categories = (
      (application.provider_application_categories as
      ...
      .filter(Boolean);
    const regions = (
      (application.provider_delivery_regions as
    ...
    return [
      ...
      {
        label: "هل يوجد توصيل؟",
        value: application.delivery_available ? "نعم" : "لا",
      },
      { label: "مناطق التوصيل", value: regions.join("، ") || "—" },
    ];
  }
  const regions = (
    (application.contractor_work_regions as
      { region_name: string }[] | undefined) ?? []
  ).map((item) => item.region_name);
  const specialties = (
    (application.contractor_specialties as
